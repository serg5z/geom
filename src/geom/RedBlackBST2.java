...
public class RedBlackBST2<Key, Value> {
     ...
     */
    public RedBlackBST2(Comparator<Key> cmp) {
    	this.cmp = cmp;
    }
    ...
    ***************************************************************************/
    ...
    private Node put(Node h, Key key, Value val) { 
        ...
        int cmp = this.cmp.compare(key, h.key);
    ...
    ***************************************************************************/
    ...
    private Node delete(Node h, Key key) { 
        ...
        if (this.cmp.compare(key, h.key) < 0)  {
            if (!isRed(h.left) && !isRed(h.left.left))
                h = moveRedLeft(h);
            ...
            if (isRed(h.left))
                h = rotateRight(h);
            if (this.cmp.compare(key, h.key) == 0 && (h.right == null))
                return null;
            if (!isRed(h.right) && !isRed(h.right.left))
                h = moveRedRight(h);
            if (this.cmp.compare(key, h.key) == 0) {
...
//    } 
    ...
    public int size(Key lo, Key hi) {
        if (this.cmp.compare(lo, hi) > 0) return 0;
    ...
    ***************************************************************************/
    ...
    private boolean isBST(Node x, Key min, Key max) {
        ...
        if (min != null && this.cmp.compare(x.key, min) <= 0) return false;
        if (max != null && this.cmp.compare(x.key, max) >= 0) return false;
