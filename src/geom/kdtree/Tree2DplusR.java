...
public class Tree2DplusR<T extends POI> {
	public Tree2DplusR(final T[] points) {
...
//		return nearest(root, q, axis);
	...
	protected void nearest(Node<T> n, Point2d q, Axis axis, PriorityQueue<Tuple2<T, Double>> items, int N) {		
		if(n != null) {
			...
			//TODO: optimize? no need to add if queue if full and distance to current is larger than largest so far		
			double d2 = distance2(n.data.location(), q);
			if((d2 <= n.R2) && (d2 < items.peek().e2)) {
				items.add(new Tuple2<T, Double>(n.data, d2));
				if(items.size() > N) {
					items.remove();
				}
			}
			System.out.println("\tafter "+n.data.location()+"@"+n.maxR2+": "+items);
			...
			if(other != null) {
				if(axis == Axis.X) {
					if((items.size() < N) || (items.peek().e2 > (other.maxR2 + ((n.data.location().x - q.x)*(n.data.location().x - q.x))))) {
						System.out.println("\t"+n.data.location()+" other side is plausible");
						nearest(other, q, axis.other(), items, N);					
					}
				} else {
					if((items.size() < N) || (items.peek().e2 > (other.maxR2 + ((n.data.location().y - q.y)*(n.data.location().y - q.y))))) {
						System.out.println("\t"+n.data.location()+" other side is plausible");
						nearest(other, q, axis.other(), items, N);					
					}
				}
	...
	private static class Node<T extends POI> {
		public Node(T data, Node<T> left, Node<T> right) {			
			this.data = data;
			this.R2 = data.R*data.R;
			this.left = left;
			this.right = right;
		}
	...
	private double augment(Node<T> n) {
		double result = 0.0;
		
		if(n != null) {
			n.maxR2 = Math.max(n.data.R*n.data.R, Math.max(augment(n.left), augment(n.right)));
			result = n.maxR2;
		}
